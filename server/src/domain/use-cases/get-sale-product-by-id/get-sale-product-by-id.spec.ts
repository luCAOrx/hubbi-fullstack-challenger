...
import { GetSaleProductByIdUseCase } from "./get-sale-product-by-id";
    ...
    await new MakeSaleFactory()
      ...
      .then(async (sale) => {
        await getSaleProductById
          ...
          .then(({ saleProducts }) => {
            deepStrictEqual(saleProducts.products?.length, 3);

            const products = saleProducts.products.map((product) => product);
